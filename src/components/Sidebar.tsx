...
export const Sidebar: React.FC<SidebarProps> = ({
    ...
    // Fermer le sidebar sur mobile lors d'un clic sur un lien
                ...
                <button
                    onClick={() => {
                        ...
                        if (!isOpen) {
                            ...
                            // Sur mobile, on ne ferme PAS le sidebar ici pour laisser l'utilisateur interagir avec le contenu de l'accordéon
                        }
                    }}
        ...
        <>
            ...
            >
                <div className="flex-grow flex flex-col p-4 overflow-y-auto custom-scrollbar min-h-0">
                    {/* Header */}
                    <header className={`flex items-center gap-4 mb-6 mt-2 flex-shrink-0 ${isSidebarOpen ? 'justify-start pl-2' : 'justify-center'}`}>
                        <div className="w-10 h-10 bg-gradient-to-br from-emerald-500 to-cyan-500 rounded-xl flex items-center justify-center shadow-lg shadow-emerald-500/20 flex-shrink-0 ring-1 ring-white/10 relative overflow-hidden group cursor-pointer hover:shadow-emerald-500/40 transition-shadow">
                            <div className="absolute inset-0 bg-white/20 translate-y-full group-hover:translate-y-0 transition-transform duration-300" />
                            <span className="text-lg font-bold text-white relative z-10">ADT</span>
                        </div>
                        {isSidebarOpen && (
                            <div className="flex-col animate-fadeIn overflow-hidden">
                                <h1 className="text-lg font-bold text-white leading-none tracking-tight">ADT</h1>
                                <p className="text-[9px] text-slate-400 font-medium uppercase tracking-widest mt-1 whitespace-nowrap">Taxi Coop Terrebonne</p>
                            </div>
                        )}
                    </header>
